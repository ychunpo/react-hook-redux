...
class ExpensiveComponent1 extends React.PureComponent {
  render() {
    const date = new Date();
    //const date = 334;
    return <h3 onClick={this.props.onClick}>{date.getSeconds()}我是一个昂贵的组件！渲染耗时！</h3>
  }
}
...
const ExpensiveComponent = React.memo(({ onClick }) => {
  const date = new Date();
  //const date = 334;
  return <h3 onClick={onClick}>{date.getSeconds()}我是一个昂贵的组件！渲染耗时！</h3>
})
...
function Com3({ p1 }) {
  ...
  console.log("Com1:", Object.is(fnn1, fn1));
  fnn1 = fn1;
  return <ExpensiveComponent1 onClick={fn1} />
}
  ...
  render() {
    return (
      <div>
        ...
        <hr />
        <h4>每次点击 fn1 都是新的</h4>
        <Com3 p1={this.state.p1} />
        <button onClick={() => this.setState({ p1: this.state.p1 + 1 })}>p1 + 1</button>
        <hr />
        <h4>不用重复生成fn1</h4>
        <Com4 p2={this.state.p2} />
        <button onClick={() => this.setState({ p2: this.state.p2 + 1 })}>p2 + 1</button>
      </div>
    );
  }
